...
# ------------------------------------------
local_dir = "mini-fineweb"  # local directory to save the data
...
def main():
    ...
    with mp.Pool(nprocs) as pool:
        ...
        for tokens in pool.imap(tokenize, fw, chunksize=16):
            ...
            if token_count + len(tokens) < shard_size:
                ...
                filename = os.path.join(
                    DATA_CACHE_DIR, f"minifineweb_{split}_{shard_index:06d}"
                )
        ...
        if token_count != 0:
            split = "val" if shard_index == 0 else "train"
            filename = os.path.join(DATA_CACHE_DIR, f"minifineweb_{split}_{shard_index:06d}")
